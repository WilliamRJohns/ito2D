...
meth1='itoeul'
meth2='euleul'
gamma=20;        %Scaling parameter for the Noise
tic
tF=25;          %length of integration in seconds
...
for alpha=[0]
    ...
    for ik=1:nr
        ...
        [eta,Ito_sum]=color_noise_2d_uniform(t,alpha,gamma);
        W=(eta/sqrt(dt));
        
        
        
        %%Compute high res rk2 solution "ground truth"
        [vorticity_full,theta_full,t_end]=run_model_switch_uniform(tF,dt,Nx,'rk2rk2',W,Ito_sum,seed);
	...
	%steps=[.001,.005,.01,.05,.1,.5,1];
        for dt=steps
            ...
            for zz=1:(size(eta,1)-jj)/jj+1
                eta2(zz,1)=mean(eta(jj*(zz-1)+1:jj*zz,:,:));
            end
            ...
            %Run Eul+ito model
            [vorticity_full,theta_full,t_end]=run_model_switch_uniform(tF,dt,Nx,meth1,W,Ito_sum,seed);
            ...
            %Run Eul model
            [vorticity_full,theta_full,t_end]=run_model_switch_uniform(tF,dt,Nx,meth2,W,Ito_sum,seed);
...
save(sprintf('avg_time_eul_%dw.mat',gamma),'avg_time_eul')
...
save(sprintf('avg_time_ito_%dw.mat',gamma),'avg_time_ito')
...
save(sprintf('avg_time_rk2_%dw.mat',gamma),'avg_time_rk2')
end
toc
%steps=[.01 steps];
