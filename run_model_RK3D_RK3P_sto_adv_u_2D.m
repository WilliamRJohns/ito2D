...
% --- nonlinear advection
    ...
    %nladvsave(jk,n)=max(abs(nonlin_adv),[],'all');
...
% --- tendency
    ...
    Cv_star = squeeze(Cv(i1,:,:) + dt/3*Cv_tendency(i1,:,:)) + dt/3*Dv_i1;
...
% --- tendency
    ...
    CT_star = squeeze(CT(i1,:,:)) + dt/3*squeeze(CT_tendency(i1,:,:))  + dt/3*Dt_i1;
...
% --- tendency
    ...
    Cv(i2,:,:) = squeeze(Cv(i1,:,:)) + dt*2/3+dt*3/2*Dv_i2;
...
% --- tendency
    ...
    CT(i2,:,:) = squeeze(CT(i1,:,:)) + dt*2/3*squeeze(CT_tendency(i2,:,:)) +dt*3/2*Dt_i2;
...
% --- tendency
    ...
    Cv_tendency(i3,:,:) = Clin_adv + Cnonlin_adv + Cb + CF + Cev + Csb;

    % integrate one time step

    Cv(i3,:,:) = squeeze(Cv(i1,:,:)) + dt/4*squeeze(Cv_tendency(i1,:,:))+ dt*3/4*squeeze(Cv_tendency(i3,:,:))...
        +dt/4*Dv_i1+dt*3/4*Dv_i3;
    
    % enforce BC

    vorticity = ifft2(squeeze(Cv(i3,:,:)),'symmetric');
    vorticity(1,:) = 0;
    vorticity(Nz/2+1,:) = 0;
    for k = Nz/2+2:Nz
        vorticity(k,:) = -vorticity(Nz-k+2,:);
    end
    Cv(i3,:,:) = fft2(vorticity);
...
% --- tendency
    clear uwindB wwindB dthetadxB dthetadzB dvortdxB dvortdzB
    CT_tendency(i3,:,:) = Clin_adv + Cnonlin_adv + Cb + CH + Ctd + Csb;

    % integrate one time step

    CT(i3,:,:) = squeeze(CT(i1,:,:)) + dt/4*squeeze(CT_tendency(i1,:,:))+3/2*dt*squeeze(CT_tendency(i3,:,:))...
        +dt/4*Dt_i1+3/4*Dt_i3;
  
    % enforce BC

    theta = ifft2(squeeze(CT(i3,:,:)),'symmetric');
    theta(1,:) = 0;
    theta(Nz/2+1,:) = 0;
    for k = Nz/2+2:Nz
        theta(k,:) = -theta(Nz-k+2,:);
